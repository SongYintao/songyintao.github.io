...
### 1.Linux概述
  ...
  程序通过把参数放入寄存器来调用系统调用，并发出陷阱指令从用户模式切换到内核模式。由于不能够用C语言写一条陷阱指令，因此系统提供了一个库，每个函数对应一个系统调用。这些函数使用汇编语言写的，不过可以从C中调用。每个函数首先将参数放到合适的地方，然后执行陷阱命令。因此，为了执行read系统调用，一个C程序需要调用read库函数。由POSIX指定的是库接口，而不是系统调用接口。POSIX规定哪些库函数是一个符合标准规范的系统必须提供的，它们的参数是什么，他们的功能是什么，以及它们返回的什么样的结果。POSIX根本没有提到真正的系统调用。

#### 1. 内核结构

![](../img/os-linux-level.png)

在图10-1中，我们看到了Linux系统的总体结构。在近一倍研究内核的组成部分，如Linux的进程调度和文件系统之前，先从整体的角度看一下Linux的内核。

内核坐落在硬件之上，负责实现与IO设备和存储管理单元的交互，并控制CPU对前述设备的访问。如下图，在最底层，内核包含中断处理程序，它们是与设备交互的主要方式，以及底层的分派机制。这种分派在中断时发生。底层的代码中止正在运行的进程，将其状态存储在内核进程结构中，然后启动相应的驱动程序。进程分派也在内核完成某些操作，并且需要再次启动一个用户进程时发生。进程分派的代码是汇编代码，并且和进程调度代码有很大不同。

![](../img/os-linux-kernal.png)

下面我们将内核子系统氛围三个主要的部件。

IO部件包含所有负责与设备交互以及实现联网和存储的IO功能的内核部件。在最高层，这些IO功能全部整合在一个虚拟文件系统层中。也就是说，从顶层来看，对一个文件进行读操作，不论是在内存还是此案中，都和从中断输入中读取一个字符是一样的。从底层来看，所有的IO操作都需要通过一个设备驱动器。所有的Linux驱动程序都可以被分类为字符驱动程序或块驱动程序，两者之间最主要的区别是块设备允许查找和随机访问而字符设备不允许。从技术上讲，网络设备实际上是字符设备，不过她们的处理和其他字符设备不太一样，因此为了清晰起见，将它们单独分类。

在设备驱动程序之上，每个设备类型的内核代码都不一样。字符设备有两种不同的使用方式。有些程序，如可是编辑器vi等，需要每一个键盘输入。原始的中断（tty）IO可以实现这种功能。其他程序，比如shell等，是面向行的，因此允许用户在输入回车并将字符串发送程序之前整行的进行编辑。

网络软件通常是模块化的，由不同的设备和协议来支持。网络设备的上一个层次负责一种馋鬼程序，确保每一个包被送到了正确的设备或协议处理器。大多数Linux系统在内核中包含一个完整的硬件路由器和功能，尽管其性能比硬件路由器的性能差一点。在路由器代码之上的是实际的协议栈，他总是包含IP和TCP协议，也包含一些其他协议。在整个网络之上的是socket接口，它允许程序来为特定的网络和协议创建socket，并为每一个socket返回一个待用的文件描述符。

在磁盘驱动器之上是IO调度器，它负责排序和分配磁盘读写操作，以尽可能减少磁头的去用移动活着满足一些其他的系统原则为方法。
